...
class Document(fitz.Document):
    ...
    def display_page(self, bar, p, display=True):
        ...
        if page_state.stale: #or (display and not write_gr_cmd_with_response(cmd)):
            ...
            logging.debug('wrote image to cmd')

        if display:  
            logging.debug('display = true')
            # clear prevpage
            self.clear_page(self.prevpage)
            # display the image
            cmd = {'a': 'p', 'i': p + 1, 'z': -1}
            success = write_gr_cmd(cmd)
            logging.debug('sent cmd to term')
            if not success:
...
def view(file_change,doc):
    ...
    logging.debug('swallow keys configured')
    bar = status_bar()
    if doc.citekey:
        bar.message = doc.citekey

    logging.debug('status bar acquired')

    count_string = ""
    stack = [0]
    keys = shortcuts() 

    logging.debug('keys() acquired')
    while True:

        bar.cmd = ''.join(map(chr,stack[::-1]))
        bar.update(doc)
        logging.debug('bar.update()')
        doc.display_page(bar,doc.page)
        logging.debug('doc.display_page()')
        ...
        logging.debug('about to getch')
        key = scr.stdscr.getch()
        # logging.debug('getch='+key)
